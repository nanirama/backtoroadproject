...
const PartsSearch =()=>{
    const collapseBtn1 = useRef(null);
    const collapseSec1 = useRef(null);
    const [value, setValue] = useState(false);
    const changeCollapseHandler=()=>{
        setValue(!value)
        if(!value)
        {
            collapseSec1.current.classList.remove('collapsed')
            collapseBtn1.current.style.backgroundImage = `url(${arrowLess})`;
            collapseBtn1.current.scrollIntoView({
                behavior: "smooth",
                block: "start",
                inline: "start"
              });
        ...
        {
            collapseSec1.current.classList.add('collapsed') 
            collapseBtn1.current.style.backgroundImage = `url(${arrowMore})`;
            collapseSec1.current.scrollIntoView({
                behavior: "smooth",
                block: "start",
                inline: "start"
              });
            ...
            <div className="container">
                ...
                <ul className="w-100 float-left d-flex flex-wrap flex-row justify-content-start align-items-stretch">
                    ...
                    {filteritems.length > numberPer && (
                        <> 
                        <div class="more-section-part collapsed" ref={collapseSec1}>
                        ...
                        <div className="btn_outer w-100 float-left text-center">
                            <Button img={arrowMore} className="btn1" ref={collapseBtn1}  onClick={changeCollapseHandler}>{value ? 'View Less' : 'View More'}</Button>
                        </div>
                        </>     
                    )}
